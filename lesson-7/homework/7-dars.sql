...
(40, 10, 1, '2023-02-10', 600.00);
 ...
 --Easy-Level
select min(Price)  from Products                     --Step 1


select max(Salary) from Employees                     --Step 2

Select count(*) from Customers                        --Step 3

select count(distinct(category)) from Products       --Step 4       

select sum(SaleAmount) from Sales                    --Step 5
where SaleID=7

select avg(Age) from Employees                      --Step 6

select DepartmentName , count(EmployeeID) from Employees    --Step 7
group by DepartmentName


select min(Category) , max(Category)  from Products           --Step 8

Select CustomerID , sum(SaleAmount) from Sales                --Step 9
...
select DepartmentName,Count(EmployeeID) from Employees        --Step 10
group by DepartmentName
...
SELECT DepartmentName ,MAX(Salary),min(Salary) from Employees     --Step 13
group by DepartmentName



SELECT DepartmentName ,avg(Salary) from Employees     --Step 14
group by DepartmentName


SELECT DepartmentName ,avg(Salary),count(*) from Employees     --Step 15
group by DepartmentName



select Category,avg(Price) from Products                  --Step 16   
group by Category
...
select year(SaleDate),sum(SaleAmount) from Sales                --Step 17
...
select  DepartmentName,Sum(Salary) from Employees                   --Step 19
GROUP BY DepartmentName
...
SELECT DepartmentName,SUM(Salary),AVG(Salary) FROM Employees                                 --Step 22
GROUP BY DepartmentName
HAVING AVG(Salary)> 65000



select CustomerID , min(TotalAmount), max(TotalAmount) from Orders                    --Step 23
GRoup by CustomerID
having max(TotalAmount)<50

select month(SaleDate) ,sum(SaleAmount),count(distinct ProductID) from Sales             --Step 24
...
select year(OrderDate) ,min(Quantity), max(Quantity) from Orders                          --Step 25
