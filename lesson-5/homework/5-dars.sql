...
SELECT ProductName AS Name                              --Step 1
FROM Products;
...
SELECT *
FROM Customers AS Client;                                                 --Step 2   Savol
...
SELECT ProductName FROM  Products                               --Step 3
UNION
...
SELECT ProductID, ProductName, Price, Category, StockQuantity          --Step 4   Savol
FROM Products
INTERSECT
SELECT ProductID, ProductName, Price, Category, StockQuantity
FROM Products_Discounted;

Select distinct FirstName,Country                                       --Step 5
from Customers

select * , case when Price>1000 then 'High'
                when Price<1000 then  'Low' end as Price_rate from Products   --Step 6



SELECT ProductID, ProductName, StockQuantity,                     --Step 7
...
CREATE TABLE OutOfStock ( ProductID INT PRIMARY KEY, 
                          ProductName VARCHAR(100), 
						  Price DECIMAL(10, 2), 
						  Category VARCHAR(50),
						  StockQuantity INT );
...
Select ProductName from Products                            --step 8
Union
Select ProductName from OutOfStock;


Select * from Products                            --step 9
Except
Select * from Products_Discounted;



select Price, iif(Price>1000,'Expensive','Affordable')  --Step 10
...
select * from Employeesb                                     --Step 11
where Age<25 and Salary >60000


update Employees set Salary =Salary * 1.10                    --Step 12
where DepartmentName = 'Salary' or EmployeeID=5 ;
...
Select *, case when SaleAmount> 500 then 'Top Tier'                            --Step 14
               when SaleAmount between 200 and 500 then 'Mid Tier'
			   else 'Low tier ' end as Tier_Rate from Sales
...
Select CustomerID from Orders                            --step 15
Except 
Select CustomerID from Invoices


select CustomerID, Quantity,case when Quantity =1 then '3%'                          --Step 16
