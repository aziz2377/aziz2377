...
CREATE TABLE Products_Discounted ( ProductID INT PRIMARY KEY, 
                                   ProductName VARCHAR(100),
								   Price DECIMAL(10, 2), 
								   Category VARCHAR(50), 
								   StockQuantity INT );
...
SELECT Category ,count(*) as TotalNumber from Products              --Task 1
group by Category


Select Category, avg(Price) as PriceAvg from Products               --Task 2
group by Category 
having Category='Electronics'


select * from Customers                                              --Task 3
where City like 'L%'

select * from Products                                              --Task 4
where ProductName like '%er'


select * from Customers                                              --Task 5
where City like '%A'


select max(Price) as HighestPrice from Products                                              --Task 6

select StockQuantity,
 iif(StockQuantity < 30,'Low Stock','Sufficient') as StockRate                       --Task 7
from Products



SELECT City ,count(CustomerID) as TotalNumber from Customers              --Task 8
group by City


select max(Quantity) as MaxQuantity,min(Quantity) as MinQuantity from Orders                --Task 9


--Medium-Level


SELECT DISTINCT CustomerID                                                                 --Task 10
FROM Orders
...
Select ProductName from Products                                               --Task 11
union all
Select ProductName from Products_Discounted

Select ProductName from Products                                               --Task 12
union 
Select ProductName from Products_Discounted


Select year(OrderDate) as OrderYear,avg(Quantity) as AvgQuantity from Orders           --Task 13
group by year(OrderDate)



select ProductName,case when Price<100 then 'Low'                                           --Task 14
              when Price Between 100 and 500 then 'Mid'
              when Price<500 then 'High' end as pricegroup 
			  from Products
			   

select City from Customers                                                                 --Task 15
order by City asc

select ProductID ,sum(SaleAmount) as SumSales from Sales                                      --Task 16
group by ProductID

Select * from Products                                                          --Task 17
where ProductName like '%oo%'


Select ProductID from Products                                               --Task 18
intersect
...
select top 3  CustomerID, sum(TotalAmount) as Totalspent                        --Task 19
from Invoices
group by CustomerID
...
Select ProductID,ProductName from Products                                               --Task 20 
except
Select ProductID,ProductName from Products_Discounted

select P.ProductName,Count(S.SaleID) TimeSold from Products as P                         --Task 21
...
select top 5 ProductID,sum(Quantity) as TotalQuantity from Orders                          --Task 22
