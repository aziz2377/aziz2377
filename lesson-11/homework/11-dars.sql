...
Select Orders.OrderID,Customers.FirstName,Customers.LastName,year(Orders.OrderDate) from Orders
...
--Task 3
select Departments.DepartmentName,Employees.Name as TopEmployee ,MaxSalary from Departments
join (Select Employees.DepartmentID ,
             max(Employees.Salary) as MaxSalary
			 from Employees
			 group by Employees.DepartmentID) as E
	 on E.DepartmentID=Departments.DepartmentID
	 join Employees 
	 on Employees.DepartmentID=E.DepartmentID and Employees.Salary=E.MaxSalary


--Task 4
Select Customers.FirstName,Customers.LastName,Orders.OrderID,year (Orders.OrderDate)  from Customers
inner join Orders
on Customers.CustomerID=Orders.CustomerID
where year (Orders.OrderDate)= 2023 and Customers.Country ='USA'

--Task 5
select Customers.FirstName,Customers.LastName,count(O.TotalOrders) as TotalOrders from Customers
join (select Orders.CustomerID,Orders.OrderID as TotalOrders from Orders
group by Orders.CustomerID,Orders.OrderID) as O
...
--Task 7
Select Customers.FirstName,Customers.LastName,O.MostRecentOrderDate,O.OrderID  from Customers
left join (select Orders.CustomerID,max(Orders.OrderDate) as MostRecentOrderDate ,Orders.OrderID from Orders
group by  Orders.CustomerID,Orders.OrderID) as O
on Customers.CustomerID=O.CustomerID
left join Orders
on Orders.CustomerID=O.CustomerID and Orders.OrderDate=O.MostRecentOrderDate
...
Select Customers.FirstName,Customers.LastName,Orders.OrderID,Orders.TotalAmount as OrderTotal from Orders
inner join Customers
on Orders.CustomerID=Customers.CustomerID
...
where year(Sales.SaleDate)=2022 and Sales.SaleAmount>400


--Task 10

Select Products.ProductName,sum(S.TotalAmount) as TotalSalesAmount from Products
inner join (select Sales.ProductID,sum(Sales.SaleAmount) as TotalAmount from Sales
group by Sales.ProductID) as S
...
Select Customers.FirstName,Customers.LastName,Orders.OrderID,Customers.Address,Orders.OrderDate from Customers
inner join Orders
on Customers.CustomerID=Orders.CustomerID
...
inner join sales 
on Products.ProductID=Sales.ProductID
where Products.ProductName='Electronics ' or Sales.SaleAmount>350


--Task 16

Select Categories.CategoryName,P.ProductCount from Categories
inner join (select  Products.Category,count(Products.ProductID) as ProductCount from Products
Group by Products.Category ) as P
on Categories.CategoryID=P.Category
group by Categories.CategoryName,P.ProductCount
...
Select Customers.FirstName,Customers.LastName,Customers.City,Orders.OrderID,Orders.TotalAmount as Amount    from Customers
inner join Orders
on Customers.CustomerID=Orders.CustomerID
...
inner join Departments
on Employees.DepartmentID=Departments.DepartmentID
where Departments.DepartmentName = 'HR' or Departments.DepartmentName = 'Finance'  and  Employees.Name LIKE '%[aeiou][aeiou][aeiou][aeiou]%'


--Task 19

select Products.ProductName, Products.StockQuantity as QuantitySold,  Products.Price from Sales
inner join Products
on Sales.ProductID=Products.ProductID
where Products.StockQuantity>100 and Products.Price>500
