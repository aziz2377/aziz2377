...
Swapped AS (
    SELECT 
        ...
        CASE 
            WHEN LEN(val) >= 2 THEN 
                SUBSTRING(val, 2, 1) + SUBSTRING(val, 1, 1) + SUBSTRING(val, 3, LEN(val))
            ELSE val
        END AS swapped_val
    FROM SplitVals
),
Recombined AS (
    SELECT 
        Id,
        STRING_AGG(swapped_val, ',') WITHIN GROUP (ORDER BY rn) AS SwappedVals
    FROM Swapped
    GROUP BY Id
)
...
SELECT
    ...
    SUM(SalesLocal) AS weekly_area_sales,
    SUM(SUM(SalesLocal)) OVER (PARTITION BY DATE_TRUNC('week', Date)) AS total_week_sales,
    ROUND(
        SUM(SalesLocal) * 100.0 / 
        SUM(SUM(SalesLocal)) OVER (PARTITION BY DATE_TRUNC('week', Date)),
        2
    ) AS area_percentage
FROM sales
GROUP BY area, DATE_TRUNC('week', Date)
ORDER BY week_start, area;
